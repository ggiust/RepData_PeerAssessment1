...
```{r check1, echo=FALSE}
# can use to verify median:
# sum(apply(ml1, 1, function(x){ ifelse(x[2] >= dailyMedian,TRUE,FALSE) }))
```
The **mean** of total steps taken per day = `r dailyMean1`.  
The **median** of total steps taken per day = `r dailyMedian1`.  
...
The 5-minute interval having the maximum average daily steps = `r maxInterval`.  
...
```
The total number of missing values in the original dataset = `r total_missing_values`.  
The **mean** of total steps taken per day = `r dailyMean2`.  
The **median** of total steps taken per day = `r dailyMedian2`.  
